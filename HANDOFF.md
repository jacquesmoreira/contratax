...
**⚠ PENDENTE DO JACQUES pra isso funcionar:** criar o webhook em **Resend → Webhooks**, apontando pra `https://www.contratax.com.br/api/webhook/resend`, evento `email.bounced`, e salvar o **Signing Secret** em `RESEND_WEBHOOK_SECRET` no Railway. Sem essa variável o endpoint responde 503 de propósito. Vale conferir também **Resend → Suppressions**: se o endereço do Marcelo estiver lá, os próximos envios nem serão tentados.
