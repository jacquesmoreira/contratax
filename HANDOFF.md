...
- **Ainda não tratado** (fora do escopo escolhido por Jacques nesta sessão): páginas de SEO programático (~500+ `/licitacoes/<categoria>/<uf>`) e blog/institucional (`/blog`, `/ajuda`, `/contato`, `/termos`, `/privacidade`). Ficam pra uma sessão futura se ele quiser.
- **Pendência em aberto pro Jacques decidir:** a calculadora de "oportunidades perdidas" (ROI) que existia antes na LP foi removida (não estava no mock) e ainda não foi recolocada em lugar nenhum — perguntar se ele quer ela de volta em algum ponto do funil.
