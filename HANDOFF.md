...
**Estado real no fim desta sessão:** 4 achados, 2 fixes de código aplicados (LIMIT em `contratosVencendo`, `--expose-gc` ativado) + 1 já corrigido antes de eu chegar (ordem do `else-if`, 11/07) + 1 achado arquitetural sem fix de código óbvio (6 loops concorrentes competindo pelo mesmo orçamento de memória, incluindo re-scan redundante do mês corrente a cada 6h no backfill). Os fixes de hoje são corretos e valem a pena, mas **não tenho certeza que eles sozinhos resolvem o custo** — o achado #4 sugere que o problema é mais estrutural (muita coisa concorrente num processo só) do que um bug pontual. Se depois de ~7 dias a taxa diária não cair de forma clara em relação aos ~$0,95/dia históricos, a próxima frente de investigação/decisão é arquitetural: espaçar melhor os loops ao longo do dia (não só no boot), evitar o re-scan integral do mês corrente no backfill a cada passada, ou considerar separar o processo em mais de um serviço (mudança maior, esbarra na limitação de volume do Railway não ser compartilhável entre serviços, documentada no próprio código).
