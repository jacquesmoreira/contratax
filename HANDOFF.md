...
**Investigação do consumo de memória (mesma madrugada):** pedido explícito do Jacques pra investigar a fundo antes de decidir sobre o Pro. Descartado: `data/capag-municipios.xlsx` (22,6MB, só usado 1x/ano por script separado, não carrega em runtime) e `src/store.mjs` (sem cache persistente em memória, cada chamada relê o arquivo do zero — não é vazamento, ainda que reparse repetido tenha custo de CPU não quantificado). **Achado real:** `contratosVencendo()` em `src/db.mjs` era a única query `SELECT * FROM contratos` do arquivo sem `LIMIT` — cliente sem UF cadastrada no perfil (chamada via `radarRenovacao()`, tanto no `GET /api/radar` quanto na exportação CSV do radar) disparava consulta nacional sem corte nenhum de estado, varrendo a tabela inteira a cada carregamento do painel. Mesmo padrão de outra query já protegida no arquivo (linha 635), com histórico documentado de OOM em produção. **Corrigido:** `LIMIT 5000` adicionado (commit `ea7a9b0`), aproveitando o `ORDER BY vigencia_fim ASC` já existente pra manter só os contratos mais urgentes. **Limite da validação:** banco local só tem 10.100 contratos (produção tem 3,1 milhões — a marca "3,1 milhões de contratos" usada na copy do site já confirma essa ordem de grandeza), então localmente a query sem UF retornou 4.777 linhas, abaixo do novo teto — não dá pra medir o ganho em dólar a partir daqui. Mas o fix é correto independente da escala: trava o teto de memória dessa query pra sempre, não importa o tamanho da tabela. Não investigado ainda (próxima sessão, se o Jacques quiser continuar): valor real de `LICITA_MEM_LIMITE_MB` em produção (não está no `.env` local), possíveis serviços duplicados/órfãos no Railway, e comportamento de memória dos loops de backfill/atualizador/colheita de itens sob carga.
