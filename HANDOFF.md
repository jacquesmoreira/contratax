...
**Conclusão prática:** a causa mais provável do custo alto de memória já estava corrigida antes desta sessão começar (11/07), não é algo que precisou de ação hoje. **A fatura do PRÓXIMO ciclo (a partir de ~05/07) deve vir bem mais barata**, já que grande parte desse ciclo roda com o auto-restart funcionando. Recomendação: Jacques conferir "Current Usage" no dashboard do Railway (uso do mês corrente, parcial) antes de decidir sobre o Pro — se já estiver desproporcionalmente menor que $31 pro mesmo número de dias, confirma a tese e não há necessidade de upgrade nem de mais investigação de código. Não investigado (baixa prioridade agora, dado o achado #2): valor real de `LICITA_MEM_LIMITE_MB` em produção, possíveis serviços duplicados/órfãos no Railway.
