...
**Estado real no fim desta sessão:** 3 achados/fixes aplicados (LIMIT em `contratosVencendo`, ordem do `else-if` já estava corrigida antes de eu chegar, `--expose-gc` agora ativo), mas a causa completa do custo AINDA não está 100% explicada — o `--expose-gc` é uma correção real e justificada (o código já pretendia fazer isso desde o início), mas não tenho certeza se sozinha ela vai derrubar a taxa observada, porque não sei se os picos do dente-de-serra são dominados pelo teto de 450MB (que o `--expose-gc` deveria aliviar antes de precisar reiniciar) ou por algo que estoura MUITO além disso rápido demais pro monitor de 5 em 5 minutos pegar (nesse caso quem mata o processo é o OOM-kill do próprio Railway, não o auto-restart do código, e `--expose-gc` ajudaria pouco). **Não confirmado ainda:** valor real de `LICITA_MEM_LIMITE_MB` em produção (perguntei ao Jacques, resposta pendente) — isso decide qual das duas explicações é a certa. Recomendação pro próximo ciclo: comparar a taxa diária de gasto de memória em ~7 dias a partir de agora (pós `--expose-gc`) contra a taxa histórica de ~$0,95/dia; se não cair de forma clara, o problema é outro (candidatos ainda não descartados: página de resposta do PNCP anormalmente grande em algum endpoint de `pncp.mjs`, ou concorrência de múltiplas requisições HTTP pesadas ao mesmo tempo).
