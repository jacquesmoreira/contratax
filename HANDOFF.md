...
**Bug de cadência descoberto e corrigido (mesmo domingo):** o Jacques perguntou como calcular quantos e-mails saem por dia dado o teto de 90/dia e a sequência de 3 etapas em 6 dias. Simulando a lógica REAL do script (não conta de cabeça), achado sério: como o loop processa a lista em ORDEM DE ARQUIVO e para ao bater 90, uma leva de gente nova (entrando em bloco) trava os 6 dias seguintes só em reenvio — 3 dias de etapa 2, depois 3 dias de etapa 3 da MESMA leva, e durante esses 6 dias **zero lead novo é tocado**. Simulação confirmou: em 40 dias, só 1.350 dos 5.018 leads (27%) recebiam qualquer coisa; no ritmo, a lista toda levaria uns 5 meses, aos trancos.

**Fix:** novo parâmetro `--novos-por-dia` (default `limiteDiario/3` = 30 com o teto atual de 90). Reenvios (etapa 2/3) continuam sem teto próprio, sempre prioritários — só a ENTRADA de gente nova (etapa 1) é racionada a 30/dia. Contador persistido em `data/campanha-contador.json` (campo novo `novos`, retrocompatível). Matemática: em regime permanente, 30 (etapa 1 de hoje) + 30 (etapa 2 de quem entrou há 3 dias) + 30 (etapa 3 de quem entrou há 6 dias) = 90/dia, constante, pra sempre — sem os buracos de antes. Resimulado após o fix: rampa 30→60→90 na primeira semana, depois 90/dia estável indefinidamente. Com 5.018 leads e 30 novos/dia, a lista toda leva ~168 dias (~5,5 meses) pra passar todo mundo pela etapa 1 — mais lento que o Jacques esperava, mas é o teto real do Resend gratuito (90/dia ÷ 3 etapas); só sobe com upgrade de plano.
...
**Investigação do consumo de memória (mesma madrugada):** pedido explícito do Jacques pra investigar a fundo antes de decidir sobre o Pro. Descartado: `data/capag-municipios.xlsx` (22,6MB, só usado 1x/ano por script separado, não carrega em runtime), `src/store.mjs` (sem cache persistente em memória, cada chamada relê o arquivo do zero), `ingest.mjs`/`backfillContratos.mjs` (ambos processam página a página via `for await`, sem acumular array grande), e o fallback nacional em `monitor.mjs` (já protegido pelo `LIMIT 8000` existente em `consultar()`).

**Achado #1 (bug real, corrigido hoje):** `contratosVencendo()` em `src/db.mjs` era a única query `SELECT * FROM contratos` do arquivo sem `LIMIT`. Cliente sem UF cadastrada disparava consulta nacional sem corte de estado a cada carregamento do painel (radar). Corrigido com `LIMIT 5000` (commit `ea7a9b0`). Banco local só tem 10.100 contratos (produção tem 3,1 milhões), então não dá pra medir o ganho em dólar localmente — mas o fix é correto independente da escala.

**Achado #2 (a causa provável de verdade, MUITO mais significativa):** o monitor de memória (`web/server.mjs`) tem um auto-restart de segurança pensado pra nunca deixar o RSS passar de ~427MB (95% de `LICITA_MEM_LIMITE_MB=450`, `process.exit(1)` → Railway reinicia limpo). Mas esse ramo crítico era um `else-if` posicionado DEPOIS do ramo de 80%, o que o tornava **matematicamente inalcançável** (RSS acima de 95% também é acima de 80%, então sempre caía no primeiro `if`) — o processo só forçava GC, nunca reiniciava sozinho, não importa o quanto a memória subisse. Achei o commit que corrigiu essa ordem: **`03b411c`, de 11/07/2026** (auditoria completa do sistema, já feita antes desta sessão, sem relação com a campanha). **A fatura de $31,15 cobrada é do período 05/06 a 05/07** — ou seja, o bug ficou ativo (sem nenhum teto real de memória) durante TODO o mês faturado, e só foi corrigido 6 dias DEPOIS do fechamento desse ciclo. A média de consumo da fatura (131.194.167 MB-min ÷ 43.200 min do mês) dá **~3.037MB em média o mês inteiro** — nível incompatível com um teto de 450MB funcionando; totalmente compatível com um teto quebrado deixando o processo crescer livre até o Railway matar por OOM. Confirmado ao vivo: `GET /health` de produção agora mostra RSS baixo logo após um restart (87MB), o que é esperado — o teste real é ver se o RSS fica estável e baixo ao longo de HORAS a partir de agora, já que o auto-restart está funcional desde 11/07.

**CORREÇÃO da conclusão acima (dado real derrubou a hipótese):** Jacques mandou o print do "Usage by Project" do ciclo atual (05/07 a hoje 20/07, 15 dias): Memory = **$13,55**. Taxa: $13,55/15 dias = **$0,90/dia**, contra $30,37/31 dias = **$0,98/dia** do ciclo anterior — praticamente a MESMA taxa (8% de diferença, dentro do ruído). O fix de 11/07 (sozinho) NÃO derrubou o custo como eu esperava. O gráfico de RAM do print mostra um padrão em dente-de-serra recorrente (sobe rápido, cai, repete, várias vezes ao longo dos 15 dias) — sinal de picos de memória rápidos e frequentes, não uma escalada lenta ao longo de um mês inteiro. Isso implica que o teto de 450MB (se é esse o valor real em produção — ainda não confirmado, `LICITA_MEM_LIMITE_MB` não está no `.env` local) está sendo estourado bem além do esperado antes de qualquer restart agir.

**Achado #3 (bug adicional, corrigido nesta sessão):** investigando por que o "força GC aos 80%" não estava suavizando os picos, achei que `global.gc()` (chamado em `web/server.mjs` desde 13/06, commit `a1b96c7`) só existe se o processo Node for iniciado com a flag `--expose-gc`. O `package.json` tinha `"start": "node web/server.mjs"` — sem a flag. Não existe `Procfile`/`railway.json`/`nixpacks.toml` sobrescrevendo isso, então em produção `global.gc` sempre foi `undefined` e aquela chamada sempre foi um no-op silencioso, **desde que o código foi escrito**, independente do bug do `else-if` (que é um problema separado, no ramo dos 95%). Corrigido: `"start": "node --expose-gc web/server.mjs"` (commit `8381135`). Testado localmente (`node --expose-gc -e "..."` confirma `typeof global.gc === 'function'`; smoke test subindo o servidor local com o novo start script, `/health` respondeu normal).

**Achado #4 (via Railway CLI, token de projeto que o Jacques gerou pra essa investigação):** instalei o Railway CLI e confirmei direto na fonte: `LICITA_MEM_LIMITE_MB` **não existe** em produção (450MB é mesmo o teto real usado pelo código) e **só existe UM serviço** no projeto (descarta serviço duplicado/órfão cobrando à toa). Vendo os logs do boot mais recente, o quadro ficou mais claro — **todos os 6 loops de fundo rodam simultaneamente no MESMO processo**: `LICITA_ATUALIZAR=1`, `LICITA_BACKFILL=1`, `LICITA_BACKUP=1`, `LICITA_CAMPANHA=1`, `LICITA_DIGEST=1`, `LICITA_ITENS_INDEX=1` — todos ativos ao mesmo tempo. O próprio código já documentava esse risco (comentário em `web/server.mjs`: "backfill INICIA 60s APOS subida do servidor pra nao competir memoria com atualizador, digest e backup no startup (causa de OOM em Hobby)") — ou seja, ESSA já era uma causa de OOM conhecida antes desta sessão, só mitigada parcialmente (atraso de 30-60s no boot, que não evita sobreposição depois que os loops emplacam).

Direto do log do boot mais recente: o `backfillLoop` processa **1.930 páginas (50 contratos cada) só do mês corrente (julho, parcial)** — e como o mês corrente NUNCA é marcado como "completo" (`mesCorrente` sempre reprocessa), esse re-scan de ~96 mil linhas se repete INTEIRO a cada passada de 6h, pra sempre, além do `ingerirNacional` dos editais rodando em paralelo logo no boot. Cada requisição/parse individual é pequena (a paginação já é feita com `for await`, sem acumular array grande — não é um "SELECT * gigante"), mas a CONCORRÊNCIA de múltiplos loops pesados de rede+parse+escrita rodando ao mesmo tempo, sem coordenação entre si além do atraso inicial, é plausivelmente o que sustenta o RSS num patamar alto com frequência — não uma única query gigante, mas carga concorrente genuína disputando o mesmo orçamento de 450MB.

Busquei também evidência direta de crash/OOM nos logs (`railway logs --filter "MEM"` e `"SIGTERM OR CRITICO OR CRASH"` nas últimas 24h): **zero ocorrências.** Isso é ambíguo — pode significar que o RSS não passou de 360MB (80%) na janela investigada (favorece a hipótese de que os fixes de hoje já bastam), OU que os picos são rápidos demais pro monitor de 5 em 5 minutos pegar antes do OOM-kill do próprio Railway agir direto (nesse caso nenhum log da aplicação chega a ser emitido). Não consegui diferenciar os dois cenários com certeza nos logs disponíveis.

**Achado #5 / fix (mesma madrugada, direto do achado #4):** o Jacques pediu pra sempre analisar tudo a fundo em decisão de custo (registrado em memória `jacques-analise-completa-custo`), então fui atrás do maior ofensor concreto que achei: o `backfillContratos()` revarria o mês corrente INTEIRO (dia 1 até hoje) a cada passada de 6h — no dia 20, isso é reprocessar as mesmas ~1.930 páginas (~96 mil contratos) só pra pegar as poucas horas novas desde a passada anterior. Antes de mexer, confirmei na especificação OpenAPI OFICIAL do PNCP (via WebFetch no `/api/consulta/v3/api-docs`) que `dataInicial`/`dataFinal` do endpoint `/v1/contratos` filtram por **Data de Publicação**, que é atribuída pelo sistema do PNCP no momento da publicação e não muda depois — não existe cenário de "contrato atrasado" reaparecendo com data anterior à consulta. Com isso confirmado, é seguro estreitar a janela do mês corrente pros últimos `margemDiasMesCorrente` dias (default 3, com folga de segurança) em vez de sempre voltar ao dia 1. Segurança extra que já existia no código (sem mudança): quando o mês vira, o fechamento ainda faz UMA varredura completa do mês que fechou antes de marcá-lo "completo" pra sempre — cobre qualquer lacuna da janela estreita. Validado com simulação (`node`, testando dias 1, 4, 5, 20, 31 e uma virada de mês): dia 20 cai de 20 pra 4 dias de janela (corte de ~80% nas páginas por passada), dia 31 cai pra ~87%. Commit `de23cda`, já em produção.

**Estado real no fim desta sessão:** 5 achados, 3 fixes de código aplicados (LIMIT em `contratosVencendo`, `--expose-gc` ativado, janela do backfill reduzida) + 1 já corrigido antes de eu chegar (ordem do `else-if`, 11/07) + confirmação de que só existe 1 serviço no Railway (sem duplicata). Dos 3 fixes de hoje, o da janela do backfill é o que tem o maior potencial de impacto real (corta ~80% do trabalho redundante do loop mais pesado), mas **ainda não dá pra confirmar em dólar** sem esperar alguns dias de fatura. Recomendação: comparar a taxa diária de gasto de memória em ~7 dias a partir de agora (20/07, pós todos os 3 fixes) contra os ~$0,95/dia históricos. Se cair de forma clara, os fixes resolveram. Se não cair, a causa é mais estrutural ainda (6 loops concorrentes disputando 450MB no mesmo processo) e a próxima decisão é maior: espaçar os loops ao longo do dia, ou separar o processo em mais de um serviço Railway (esbarra na limitação de volume não-compartilhável entre serviços, documentada no próprio código).

**Token do Railway CLI usado nesta sessão:** token de PROJETO (não de conta), gerado pelo Jacques especificamente pra essa investigação. Ele pode revogar em Railway → Contratax → Settings → Tokens quando quiser.
...
**Desdobramento: o e-mail do Marcelo ESTÁ VOLTANDO (bounce).** Depois do fix do digest, disparei o envio manual (`POST /api/admin/digest`) e saiu pros 4 clientes com acesso: Atlântica (49 novos), Jacques (1), DRS (73) e Marcelo (15). Só que o dele **bounceou**, os outros 3 entregaram. Detalhe do Resend: tipo `Transient` / subtipo `General`, e a sequência foi `Sent → Delivered → Bounced` (o Gmail aceitou e devolveu depois), padrão típico de **caixa cheia**. Não é bloqueio do nosso domínio (dois outros Gmail no mesmo lote chegaram) nem endereço inválido (ele envia daquele endereço). Como é temporário, não desativamos nada: pode voltar sozinho. A `RESEND_API_KEY` de produção é restrita a envio, então não dá pra consultar bounce pela API, só pelo painel do Resend.

**Correção estrutural (commit `66c9016`):** o sistema não tratava bounce de forma nenhuma, então esse cliente ficaria indefinidamente sem receber e só saberíamos por outro chamado. Criado `src/bounces.mjs` + `POST /api/webhook/resend` (assinatura Svix verificada com HMAC-SHA256 sobre o corpo cru, sem dependência externa; anti-replay de 5min; falha fechada sem segredo, mesmo critério do webhook do Asaas). O evento marca `_emailBounce` no perfil e alerta o admin por e-mail, com destaque quando é pagante e throttle de 12h por endereço. Endereço de campanha fria (sem conta) não gera alerta. O painel do cliente ganhou um banner avisando que os e-mails estão voltando, com link pra trocar o endereço, já que o e-mail (canal normal de aviso) por definição não chega nele. Testado com servidor local nos 5 casos de segurança + falha fechada + processamento real do evento.

**Webhook CONFIGURADO e verificado (21/07):** o Jacques criou o webhook no Resend e a variável `RESEND_WEBHOOK_SECRET` no Railway. Confirmado em produção: o endpoint saiu de `503 webhook nao configurado` pra `401 sem assinatura`, ou seja, o segredo carregou e a verificação está ativa (evento real do Resend vem assinado e passa).

**Limpeza automática do aviso (commit `2eb62ac`):** o evento `email.delivered` agora limpa o `_emailBounce`, fazendo o banner sumir do painel sem intervenção. **O ponto delicado é a ORDEM:** como o bounce real foi `Sent → Delivered → Bounced`, um evento de entrega pode ser mais VELHO que o bounce, e limpar em qualquer entrega apagaria um aviso ainda válido (também se o Resend reenviar eventos fora de ordem). Por isso a limpeza só ocorre quando a entrega é comprovadamente mais nova, comparando o `created_at` dos eventos; o bounce passou a gravar a data do EVENTO, não a do processamento. Limpar também libera o throttle de 12h do alerta ao admin. Testado com perfis em memória nos 6 casos, incluindo o crítico (entrega anterior ao bounce não limpa).

**⚠ PENDENTE DO JACQUES:** adicionar o evento **`email.delivered`** ao webhook já criado no Resend (hoje só tem `email.bounced`), senão o banner nunca some sozinho.

**Sobre Resend → Suppressions:** o menu não existe em todas as contas (depende do plano/versão do painel) e o Jacques não achou. Não é bloqueio: o teste do dia mostrou que o envio pro Marcelo saiu normalmente e chegou a marcar `Delivered` antes de bouncear, então o Resend não está suprimindo o endereço. O problema é do lado do Gmail dele.
...
### 2026-08-08 (fim) — pente fino da conversão + varredura competitiva

**Pente fino (commit `7e6eb6b`), 6 furos corrigidos:** (1) ZERO garantia em qualquer lugar → agora "se não fizer sentido, a gente devolve" (7 dias) na página de assinar e no paywall; (2) excesso de escolha: a página abria com 4+2 planos e DESTACAVA o mais caro (Expertise R$397) → agora abre com Starter+Básico, Básico "Recomendado", volume atrás de link; (3) **o Starter R$59 NÃO aparecia no paywall** (só Básico R$149 e Pro R$247), escondendo o degrau mais convertível no momento da decisão → corrigido; (4) copy quebrada "analise dos editais: le o edital" em 4 pontos → reescrita por resultado; (5) **teste 7→14 dias** (ganhar licitação leva semanas; reversível por `LICITA_TRIAL_DIAS`, só afeta cadastros novos) + copy da LP/comparativo atualizada, mantendo os 7 dias do CDC e da garantia; (6) a régua de e-mail viraria MENTIRA com trial maior ("termina em 24h" com 8 dias pela frente) → avisos de fim agora são relativos ao que FALTA (<=2 e <=1 dia), funcionam com qualquer duração.

**Varredura competitiva (pedido do Jacques).** ConLicitação: 25 anos, 6.000 fontes, 0800+WhatsApp visíveis, alerta por WhatsApp, **15 dias** de teste, consultoria/jurídico. Effecti: **+3.000 clientes, +178 mil propostas vencedoras, +R$82 bi arrematados, case Boston Scientific**, robô de lances, "autodemonstração em 2 minutos". **Nossos gaps reais:** zero prova social de clientes (não inventar; usar os números que temos: 3,1M contratos, 27 mil editais, 5.570 municípios), só e-mail como canal (eles têm WhatsApp), sem canal humano visível.
