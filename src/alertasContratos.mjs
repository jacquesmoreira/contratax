// Alertas automáticos por e-mail para CONTRATOS que estão se aproximando do fim.
//
// Marcos: 90, 60, 30 dias antes do data_fim. Cada contrato recebe no máximo 1
// e-mail por marco. Roda no atualizarLoop.
//
// CORREÇÃO 09/08/2026: o arquivo inteiro estava sem acentuação (bug antigo,
// não estilo) — saía "orgao", "voce", "prorrogacao", "Termino" pro cliente
// pagante, num alerta ligado direto ao faturamento dele.
...
function htmlAlerta(perfil, contrato, marco, baseUrl) {
  const titulo = marco === 90 ? "Seu contrato vence em 90 dias, hora de pensar em prorrogação"
              : marco === 60 ? "Seu contrato vence em 60 dias, prepare a minuta de prorrogação"
              :                "Seu contrato vence em 30 dias, protocole o pedido de prorrogação agora";
  ...
  return `<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:540px;margin:0 auto;padding:30px 24px;color:#0f172a">
    ...
    <p>Seu contrato com o órgão <b>${contrato.orgao_nome || "público"}</b> termina em <b>${marco} dias</b>. Quanto mais cedo você protocola o pedido de prorrogação ou negocia o aditivo, maior a chance de manter o faturamento sem interrupção.</p>
    <table style="width:100%;border-collapse:collapse;margin:16px 0;background:#f8fafc;border-radius:10px;overflow:hidden">
      ...
      <tr><td style="padding:10px 14px;font-size:13px;color:#475569">Término</td><td style="padding:10px 14px;font-size:13px">${new Date(contrato.data_fim).toLocaleDateString("pt-BR")}</td></tr>
    </table>
    <a href="${baseUrl}/contratos?c=${perfil.token}" style="display:inline-block;background:#4338ca;color:#fff;text-decoration:none;padding:13px 26px;border-radius:9px;font-weight:700;margin:14px 0 0 0">Gerar minuta de prorrogação</a>
    <p style="color:#94a3b8;font-size:12px;margin-top:28px">ContrataX, gestão de contratos públicos</p>
  </div>`;
}
